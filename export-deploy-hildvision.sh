...
# Konfiguration - mit absoluten Pfaden für Zuverlässigkeit
SYNOLOGY_USER="comabo47"
SYNOLOGY_HOST="192.168.1.183"
SYNOLOGY_PORT="28"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SYNOLOGY_SSH_KEY="$SCRIPT_DIR/synology"
LOCAL_REPO="$SCRIPT_DIR"
LOCAL_PUBLIC="$LOCAL_REPO/public"
TEMP_DIR="/tmp/hildvision-export-$$"
LOG_FILE="$SCRIPT_DIR/hildvision-deploy.log"

# Remote Verzeichnis auf Synology (im Home-Dir des Benutzers, nicht in /volume1/)
SYNOLOGY_EXPORT_SUBPATH="hildvision-export/192.168.1.183/wordpress"

SSH_OPTS=(-i "$SYNOLOGY_SSH_KEY" -p "$SYNOLOGY_PORT" -o StrictHostKeyChecking=no -o IdentitiesOnly=yes -o BatchMode=yes)
...
log "✅ SSH-Schlüssel gefunden: $SYNOLOGY_SSH_KEY"
...
# 2. Auf Synology den Export durchführen (im Home-Verzeichnis)
log "🔄 Starte WordPress Export auf Synology..."
ssh "${SSH_OPTS[@]}" "$SYNOLOGY_USER@$SYNOLOGY_HOST" << 'SYNOLOGY_CMD'
    # Altes Export-Verzeichnis löschen (im Home-Verzeichnis)
    rm -rf ~/hildvision-export
    mkdir -p ~/hildvision-export
    cd ~/hildvision-export
...
# 3. Dateien vom Synology herunterladen (via TAR über SSH statt rsync,
#    da DSM 'rsync --server' über SSH blockiert)
log "📥 Kopiere Dateien vom Synology (tar über SSH)..."
ssh "${SSH_OPTS[@]}" "$SYNOLOGY_USER@$SYNOLOGY_HOST" \
    "tar czf - -C ~/$SYNOLOGY_EXPORT_SUBPATH ." | tar xzf - -C "$TEMP_DIR/"

log "✅ Download erfolgreich"

# Prüfe ob überhaupt Dateien da sind
DOWNLOADED_COUNT=$(find "$TEMP_DIR" -type f 2>/dev/null | wc -l)
log "📊 Heruntergeladene Dateien: $DOWNLOADED_COUNT"
if [ "$DOWNLOADED_COUNT" -eq 0 ]; then
    log "❌ Keine Dateien heruntergeladen!"
    exit 1
fi
